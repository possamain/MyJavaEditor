...
public class RunCMD {
	...
	public RunCMD(File file) {
		String cmdCommand;
		try {
			//Windo
			Runtime.getRuntime().exec(new String[] { "cmd.exe", "/c", "start" });
			//Runtime.getRuntime().exec(new String[] {"file.bat", "/c"});

			setBot(new Robot());
			getBot().setAutoDelay(5);
			getBot().delay(1000);
			...
			for (int i = 0; i < cmdCommand.length(); i++)
				pressKey(Character.toString(cmdCommand.charAt(i)));
			pressKey("ENTER");
			...
			getBot().delay(1000);
			cmdCommand="java ";
			cmdCommand += file.getName().substring(0, file.getName().length()-5);
			System.out.println(cmdCommand);
	...
	public static void setBot(Robot r) {
		RunCMD.r = r;
	}
}
